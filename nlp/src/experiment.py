...
class Experiment:
    ...
    def get_classifier_search_space(
        self, trial: optuna.trial.Trial, classifier_name: str
    ) -> dict:
        ...
        if classifier_name == "SVC":
            search_space = {
                ...
                "model": SVC(gamma="auto", random_state=self.random_state),
            }
            ...
            raise ValueError(
                f"Model {classifier_name} not suported. Suported models are: {model_choices}"
            )
        return search_space
    ...
    def objective(
        ...
        Returns:
            float: F1 score.
        """
        classifier_name = trial.suggest_categorical("classifier", self.model_choices)
        search_space_classifier = self.get_classifier_search_space(
            trial, classifier_name=classifier_name
        )
        ...
        if vectorize:
            ...
            preprocessing_params = {
                ...
                "limit_consecutive_chars": trial.suggest_int(
                    "limit_consecutive_chars", 0, 1
                ),
            }
            pipeline = Pipeline(
                [
    ...
    def run_experiment(
        ...
        """
        objective = partial(
            self.objective, X=X, y=y, vectorize=vectorize, prefix=prefix
        )
